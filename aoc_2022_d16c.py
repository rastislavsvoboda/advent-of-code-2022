...
import math
import sys
print(sys.setrecursionlimit(3900))
...
def getVal(score,p,pm,pe,t,O,G,R,L,DP,tM):
    # orded = "".join([a + "_" + str(b) +";" for (a,b) in sorted(O)])
    orded = frozenset(O)
    tLeft = tM - t + 1
    if (score,p,pm,pe,tLeft,orded) in DP:
        return DP[(score,p,pm,pe,tLeft,orded)]
    ...
    if tLeft == 0:
        if p == 0:
            ...
            res = getVal(score,p-1,pm,pe,t,O,G,R,L,DP,tM)

        # print(res)
    # elif len(O) == L:
    #     # res = getVal(score,pm,pe,t+1,O,G,R,L,DP,tM)
    #     s = sum(R)
    #     res = s * tLeft
    else:
        pos_m = G[pm]
        ...
        if p == 1:
            ...
            if R[pm] == 0:
                ...
                newO = O[:]
                newO.append(pm)
                res = max(res, R[pm] * tLeft + getVal(score,p,pm,pe,t+1,newO,G,R,L,DP,tM))            

            for npm in pos_m:
                res = max(res, getVal(score,p,npm,pe,t+1,O,G,R,L,DP,tM))
        else:
            opened = [o for o in O]
            if R[pe] == 0:
                pass
                ...
                newO = O[:]
                newO.append(pe)
                res = max(res, R[pe] * tLeft + getVal(score,p,pm,pe,t+1,newO,G,R,L,DP,tM))

            for npe in pos_e:
                res = max(res, getVal(score,p,pm,npe,t+1,O,G,R,L,DP,tM))

    assert res is not None
    DP[(score,p,pm,pe,tLeft,orded)] = res
...
def solve2(lines):
    ...
    print("xxx")
    res = getVal(0, 1,"AA","AA",1,[],G,R,L,DP,26)
    print("res", res)
    print("yyy")
