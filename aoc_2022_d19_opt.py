...
def play(b, max_minutes):
    ...
    while len(q):
        if len(q) % 1000000 == 0:
            print(len(q), len(SEEN), max_geode)
        ...
        can_build_str = "".join([str(new_robot) for new_robot in [
                                ROB_GEO, ROB_ORE, ROB_CLA, ROB_OBS] if can_build(COS, game_data, new_robot)])
        ...
        if (new_game, can_build_str, t-1) not in SEEN:
...
print(CGRN + "puzzle:", solve2(lines_puzzle), CEND)  # 21546 (= 21 * 27 * 38)
