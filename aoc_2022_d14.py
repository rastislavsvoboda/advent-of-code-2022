...
def move1(G, s, R):
    ...
    x = G.get((r+1, c), ".")
    if x == ".":
        return move1(G, (r+1, c), R)
    xx = G.get((r+1, c-1), ".")
    if xx == ".":
        return move1(G, (r+1, c-1), R)
    xxx = G.get((r+1, c+1), ".")
    if xxx == ".":
        return move1(G, (r+1, c+1), R)
    G[(r, c)] = "o"
...
def move2(G, s, R):
    ...
    x = G.get((r+1, c), "#" if r == R+1 else "." )
    if x == ".":
        return move2(G, (r+1, c), R)
    xx = G.get((r+1, c-1), "#" if r == R+1 else ".")
    if xx == ".":
        return move2(G, (r+1, c-1), R)
    xxx = G.get((r+1, c+1), "#" if r == R+1 else ".")
    if xxx == ".":
        return move2(G, (r+1, c+1), R)
    G[(r, c)] = "o"
    return ((0, 500), False, False)
...
print(CRED + "sample:", solve2(lines_sample), CEND)  # 93
print(CGRN + "puzzle:", solve2(lines_puzzle), CEND)  # 26683
